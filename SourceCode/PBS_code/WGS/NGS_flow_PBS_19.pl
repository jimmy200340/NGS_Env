...
#Run path
my $path = "/work2/u00ccl01/NV0047_0131";
...
#sample names
#chdir $path;
my @sample = ("NV0047-10_S21");
#my @sample = `cd $path ; ls | cut -d "_" -f1,2 | uniq`;
...
#genome to use
my $genome = $UCSC_Ref_19;

#freec_conf
foreach $sample(@sample)
{
chomp $sample;
system ("mkdir $path/$sample");
chdir $sample;
mkdir freec;
...
chrLenFile = /project/GP1/reference/Homo_sapiens/UCSC/hg19/Sequence/WholeGenomeFasta/genome.fa.fai
ploidy = 2
breakPointThreshold =0.08
window = 10000 
chrFiles = /project/GP1/reference/Homo_sapiens/UCSC/hg19/Sequence/Chromosomes 
...
foreach $sample(@sample)
{
	...
	(
		...
		cmd => "cd $path ; ~/bin/bwa mem -M -R $readgroup -t $core -K 10000000 $reference $path/$sample\_R1_001.fastq.gz $path/$sample\_R2_001.fastq.gz | $samblaster -M -e | $samtools view -bS -o $path/$sample.bam -",
	);
