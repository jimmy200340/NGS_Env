...
my $ref38_all = "/pkg/biology/reference/Homo_sapiens/NCBI/GRCh38/Sequence/BWAIndex/genome.fa";
...
#sample names
chdir $path;
#my @sample = `ls | cut -d "." -f1 | uniq`;
my @sample = ("NV0067-03","NV0067-04","NV0067-05","NV0067-06","NV0067-07");
...
chrLenFile = /work2/jimmy200340/Reference/ref_GRCh38/genome.fa.fai
ploidy = 2
breakPointThreshold =0.08
window = 10000 
chrFiles = /work2/jimmy200340/Reference/ref_GRCh38
numberOfProcesses = 40
sambamba = /pkg/biology/sambamba/sambamba_v0.6.8/sambamba
SambambaThreads = 40
EOF
...
foreach $sample(@sample)
{
	...
	(
		...
		cmd => "cd $path ; ~/bin/bwa mem -M -R $readgroup -t $core -K 10000000 $reference $path/$sample.R1.fastq.gz $path/$sample.R2.fastq.gz | $samblaster -M -e | $samtools view -bS -o $path/$sample.bam -",
	);
	...
	(
		...
		cmd => "cd $path/$sample/clineff ; /pkg/biology/Java/jdk_10.0.2/bin/java -jar ClinEff.jar -l /work2/jimmy200340/license/clinEff.license -v $genome $path/$sample/StrelkaGermlineWorkflow/results/variants/variants.vcf.gz > ann.vcf",
	);
	...
	(
		...
		project => "MST107119",
		group => "MST107119",
		cmd => "cd $path ; mv $sample*.std $path/$sample/log ; mv $sample*.err $path/$sample/log",
	);
	
	#Post_FREEC
	my $sample_postfreec = PBS::Client::Job->new
	(
		queue => 'nhri192G',
		name => "$sample.postfreec",
		project => "MST107119",
		group => "MST107119",
		ofile => "$sample.postfreec.std",
		efile => "$sample.postfreec.err",
		cmd => "cd $path/$sample/freec ; cat /pkg/biology/FREEC/FREEC_v11.5/scripts/assess_significance.R | /pkg/biology/R/R_default/bin/R --slave --args $sample.mark.sort.bam_CNVs $sample.mark.sort.bam_ratio.txt ; cat /pkg/biology/FREEC/FREEC_v11.5/scripts/makeGraph.R | /pkg/biology/R/R_default/bin/R  --slave --args 2 $sample.mark.sort.bam_ratio.txt",
	);
	...
	#$sample_clineff->next({ok => $sample_mvlog});
	$sample_freec->next({ok => $sample_mvlog});
	$sample_mvlog->next({ok => $sample_postfreec});
